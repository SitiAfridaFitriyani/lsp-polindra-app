    @can('asesi')
        ...
        <script>
            ...
            function saveSignature() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('userTestWawancara.asesi-signature') }}',
                    headers: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    jawaban.forEach(item => {
                        const radioButtons = document.querySelectorAll(`input[name="status_assesmen[${item.test_wawancara_id}]"]`);
                        const tanggapan = document.getElementById(`tanggapan[${item.test_wawancara_id}]`);
                        const umpanBalik = document.getElementById(`umpan_balik[${item.test_wawancara_id}]`);

                        radioButtons.forEach(radio => {
                            if (radio.value === item.status_assesmen) {
                                radio.checked = true;
                            }
                        });
                        if(item.tanggapan) {
                        ...
                        if(item.umpan_balik) {
                            umpanBalik.value = item.umpan_balik;
                        }
                    });
                },
                error: function(xhr, status, error) {
